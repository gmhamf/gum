...
<div class="card">
    <div class="card-header">
        <h4>
            ...
            <span data-en="Members List" data-ar="قائمة الأعضاء">Members List</span>
        </h4>
        <a href="{{ route('gym.members.create') }}" style="background: linear-gradient(135deg, #FFD700, #D4AF37); color: #000; padding: 0.5rem 1rem; border-radius: 8px; text-decoration: none; font-weight: 600; display: inline-flex; align-items: center; gap: 0.5rem;">
    ...
    <div class="card-body">
            ...
            <table style="width: 100%; border-collapse: collapse; color: #ccc;">
                <thead>
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.1); text-align: left;">
                        ...
                        <th style="padding: 1rem 0.5rem;" data-en="Code" data-ar="الكود">Code</th>
                        <th style="padding: 1rem 0.5rem;" data-en="Phone" data-ar="الهاتف">Phone</th>
                        <th style="padding: 1rem 0.5rem;" data-en="Trainer" data-ar="المدرب">Trainer</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                        ...
                        <td style="padding: 1rem 0.5rem;">
                            <div style="font-weight: 600; color: #fff;">{{ $member->name }}</div>
                            <small style="color: #666;">{{ $member->email }}</small>
                        </td>
                        <td style="padding: 1rem 0.5rem;">
                            <span style="font-family: monospace; background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px;">{{ $member->member_code }}</span>
                        </td>
                        <td style="padding: 1rem 0.5rem;">{{ $member->phone }}</td>
                        <td style="padding: 1rem 0.5rem;">{{ $member->trainer->name ?? '---' }}</td>
                        <td style="padding: 1rem 0.5rem;">
                            @if($member->status)
                        ...
                        <td style="padding: 1rem 0.5rem;">
                            <div style="display: flex; gap: 0.5rem;">
                                <a href="{{ route('gym.members.edit', $member->id) }}" style="color: #D4AF37; padding: 0.4rem; border-radius: 4px; background: rgba(212,175,55,0.1);" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="{{ route('gym.members.destroy', $member->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
        ...
        <div style="text-align: center; padding: 3rem; color: #666;">
            <i class="fas fa-user-friends" style="font-size: 3rem; margin-bottom: 1rem; display: block; opacity: 0.5;"></i>
            <p data-en="No members found." data-ar="لا يوجد أعضاء.">No members found.</p>
        </div>
        @endif
    </div>
</div>
